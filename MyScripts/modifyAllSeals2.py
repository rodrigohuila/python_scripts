...
def main():  # MAIN
        ...
        if ext == '.xls':
            ...
            for i in range(sheet.nrows):  # Loop through the sheet
                ...
                sello2 = (repr(sheet.cell_value(i, col2))).replace(' ', '').replace(
                    '-', '').replace("'", "").replace('_', '').strip()
                # sello3 = (repr(sheet.cell_value(i, col3))).replace(' ', '').replace(
                #    '-', '').replace("'", "").replace('_', '').strip()
                ...
                if ((sello1[0:2]).isdecimal != True) or ((sello2[0:2]).isdecimal != True):
                    # Vacíos nooooooo
                    if sello2.find(sello1[0:6]) != -1 and sello1 != '0':
                        # Modify cell if is not match
                        sheet2.write(i, col1, '0')
                    # Vacíos nooooooo
                    elif sello1.find(sello2[0:6]) != -1 and sello2 != '0':
                        # Modify cell if is not match
                ...
                if ((re.compile(massello)).match(sello3)) != None:
                    ...
                    if massello1 != '0' or massello2 != '0':
                        sello3 = (sello3.replace(
                            massello, '').replace('.', ''))
                        sheet2.write(i, col3, (sello3.lstrip()))  # Modify cell
                        print(sello3)
                ...
                # Condición extrema busca si un sello esta en más sellos
                if ((re.compile(sello3)).match(massello1)) != None:
                    # print((re.compile(massello1)).match(sello3))
                    sello3 = (sello3.replace(massello1, ''))
                    sheet2.write(i, col3, (sello3.lstrip()))  # Modify cell
                    print(sello3)
                elif ((re.compile(sello3)).match(massello2)) != None:
                    # print((re.compile(massello1)).match(sello3))
                    sello3 = (sello3.replace(massello2, ''))
                    sheet2.write(i, col3, (sello3.lstrip()))  # Modify cell
                    print(sello3)
    ...
    # Print Results
    print('\nTotal de unidades Preestibadas: ' + str(contador))
    print('\nArchivos procesados: ')
